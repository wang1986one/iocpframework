	...
	*/
	...
	namespace thread 
	{
		...
		namespace detail
		{
			...
			// handler 分配器
			template<typename HandlerT>
			detail::ThreadParamBase *_HandlerAllocate(const HandlerT &handler)
			{
				typedef detail::ThreadParam<HandlerT> Thread;
				void *p = HandlerAllocateHook(sizeof(Thread), &handler);
				new (p) Thread(handler);

				return static_cast<detail::ThreadParamBase *>(p);
			}
		...
		{
		private:
			detail::ThreadParamBase *callback_;
		...
		public:
			...
			{
				if( callback_ != 0 )
					detail::_HandlerDeallocate(callback_);
			}
		public:
			template<typename HandlerT>
			...
			{
				assert(callback_);
				return TRUE == ::QueueUserWorkItem(&QueueWorkItemPool::WorkerThreadProc, callback_, nFlags);
			}
		...
		public:
			...
			bool Call(DWORD dwDueTime, DWORD dwPeriod, ULONG nFlags = WT_EXECUTEDEFAULT)
			{
				...
				return TRUE == ::CreateTimerQueueTimer(&newTimer_, NULL, 
					...
					callback_, dwDueTime, dwPeriod, nFlags);
			}
		...
		public:
			...
			bool Call(HANDLE hObject, ULONG dwWait = INFINITE, ULONG nFlags = WT_EXECUTEDEFAULT)
			{
				...
				return TRUE == ::RegisterWaitForSingleObject(&waitObject_, hObject, 
					&WaitObjectPool::WorkerThreadProc, callback_, dwWait, nFlags);
			}	
			// 取消
