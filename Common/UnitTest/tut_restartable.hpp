...
namespace tut
{
	...
	namespace util
	{
		...
		*/
		void Serialize(std::ostream& os, const tut::TestResult& tr)
		{
	...
	*/
	...
	public:
		...
		*/
		RestatableWrapper(const std::string& dir = ".")
			: runner_(runner.get())
			, callbacks_()
			, dir_(dir)
			, log_( dir + '/' + "log.tut" )
	...
	private:
		...
		*/
		void RegisterExecution(const std::string& grp, int test) const
		{
			// last executed test pos
		...
		* Read log file
		*/
		void _ReadLog(std::string& fail_group, int& fail_test) const
		{
