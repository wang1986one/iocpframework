...
namespace DataStructure
{
	...
	namespace allocators
	{
		...
		template< size_t Size, typename NeltsT = MaxNelts<> >
		class CacheSuballoc
		{
		private:
		...
		template<typename T, 
			typename MaxT = MaxNone,
			template< size_t, typename > class CacheT = CacheFreelist,
			template< typename > class SyncT = SyncShared >
		class Allocator
			//: public SyncT< CacheT< sizeof(T), MaxT > >			// 潜在空基类优化
		{
		public:
			...
			typedef CacheT< sizeof(T), MaxT >				CacheType;
			typedef SyncT< CacheType >						SyncType;
			typedef Allocator< T, MaxT, CacheT, SyncT >		ThisType;
			

			template<typename U>						
			struct rebind								
			{ typedef Allocator<U, MaxT, CacheT, SyncT> other; };
		...
		public:
			...
			//template<typename U>
			Allocator(const Allocator<T, MaxT, CacheT, SyncT> &rhs)
				: sync_(rhs.sync_)
			{}
		...
		public:
			...
			pointer allocate(size_type sz)
			{
				...
				if( sz == 1 )
					return static_cast<pointer>(sync_.allocate(sizeof(value_type)));
				else
			...
			void deallocate(pointer ptr, size_type sz)
			{
				if( ptr == 0 )
					...
					sync_.deallocate(ptr, sizeof(value_type));
				else
					::operator delete(ptr);
			}
		...
		template<typename MaxT,
			template< size_t, typename > class CacheT,
			template< typename > class SyncT>										
		class Allocator<void, MaxT, CacheT, SyncT>								
		{												
		public:											
		...
		public:											
			Allocator() {}									
			template<typename T>						
			Allocator(const Allocator<T, MaxT, CacheT, SyncT> &) {}					
			template<typename T>						
			Allocator &operator=(const Allocator<T, MaxT, CacheT, SyncT> &)			
			{ return *this; }	
		...
		inline bool operator==(const AllocatorT &lhs, const AllocatorT &rhs)
		{
		...
		/*
		typedef Allocator< T, MaxUnbounded >::AllocType											AllocatorUnbounded;
		typedef Allocator< T, MaxFixedSize< 10 > >::AllocType									AllocatorFixedSize;
		typedef Allocator< T, MaxVariableSize >::AllocType										AllocatorVariableSize;

		typedef Allocator< T, MaxNelts<>, CacheSuballoc<sizeof(T)> >::AllocType			AllocatorSuballoc;
		typedef Allocator< T, MaxNelts<>, CacheChunklist<sizeof(T)> >::AllocType		AllocatorChunklist;
		*/
